...
pkg_list = c("ggplot2", "metafor", "readxl", "weights", "latex2exp", "ggpubr", 
             "shades", "ggnewscale", "scales", "ggsignif", "colormap","stringi","magick")
...
dev.off()
save_as_pdf(sprintf("%s/forest_vegfr1vegf165_kon", results_path), width=4)
...
dev.off()
save_as_pdf(sprintf("%s/forest_vegfr2vegf165_kon", results_path), width=4)
...
dev.off()
save_as_pdf(sprintf("%s/forest_nrp1vegf165_kon", results_path), width=4)
...
dev.off()
save_as_pdf(sprintf("%s/forest_vegfr1vegf165_koff", results_path), width=4)
...
dev.off()
save_as_pdf(sprintf("%s/forest_vegfr2vegf165_koff", results_path), width=4)
...
dev.off()
save_as_pdf(sprintf("%s/forest_nrp1vegf165_koff", results_path), width=4)
...
# For error bar
bar_data_kon = data.frame(
  Source = c("VEGFR1", "VEGFR2", "NRP1"),
  Average = c(rm_vegfr1_kon$b, rm_vegfr2_kon$b, rm_nrp1_kon$b),
  lb = c(rm_vegfr1_kon$b - rm_vegfr1_kon$se,
         rm_vegfr2_kon$b - rm_vegfr2_kon$se,
         rm_nrp1_kon$b - rm_nrp1_kon$se),
  ub = c(rm_vegfr1_kon$b + rm_vegfr1_kon$se,
         rm_vegfr2_kon$b + rm_vegfr2_kon$se,
         rm_nrp1_kon$b + rm_nrp1_kon$se),
  color = c("darkblue", "darkgreen", "darkred")
)

bar_data_koff = data.frame(
  Source = c("VEGFR1", "VEGFR2", "NRP1"),
  Average = c(rm_vegfr1_koff$b, rm_vegfr2_koff$b, rm_nrp1_koff$b),
  lb = c(rm_vegfr1_koff$b - rm_vegfr1_koff$se,
         rm_vegfr2_koff$b - rm_vegfr2_koff$se,
         rm_nrp1_koff$b - rm_nrp1_koff$se),
  ub = c(rm_vegfr1_koff$b + rm_vegfr1_koff$se,
         rm_vegfr2_koff$b + rm_vegfr2_koff$se,
         rm_nrp1_koff$b + rm_nrp1_koff$se),
  color = c("darkblue", "darkgreen", "darkred")
)
...
set.seed(1991)
p = ggplot() +
  geom_point(data = vegfr1, aes(x = "VEGFR1", y = kon, colour = Reference), size = 7, position = position_jitter(width=0.1, height=0)) +
  ...
  new_scale_color() + 
  geom_point(data = vegfr2, aes(x = "VEGFR2", y = kon, colour = Reference), size = 7, position = position_jitter(width=0.1, height=0)) +
  ...
  new_scale_color() +
  geom_point(data = nrp1, aes(x = "NRP1", y = kon, colour = Reference), size = 7, position = position_jitter(width=0.1, height=0)) +   # Add NRP1 kon plot
  ...
  geom_errorbar(data = bar_data_kon, aes(x = Source, ymin = lb, ymax = ub), colour = bar_data_kon$color, width=0.2, linewidth=1) +
...
dev.off()
save_as_pdf(sprintf("%s/kon", results_path), width=6)
...
#koff
p_koff = ggplot() +
  geom_point(data = vegfr1, aes(x = "VEGFR1", y = koff, colour = Reference), size = 7, position = position_jitter(width=0.1, height=0)) +
  geom_point(data = vegfr1, aes(x = "VEGFR1", y=rm_vegfr1_koff$b), shape = 95, size=20, colour = "darkblue") +
  labs(color="VEGFR1") +
  lightness(scale_color_brewer(palette="Blues"), scalefac(0.8)) +
  guides(color = guide_legend(order=1)) +
  new_scale_color() + 
  geom_point(data = vegfr2, aes(x = "VEGFR2", y = koff, colour = Reference), size = 7, position = position_jitter(width=0.1, height=0)) +
  geom_point(data = vegfr2, aes(x = "VEGFR2", y=rm_vegfr2_koff$b), shape = 95, size=20, colour = "darkgreen") +
  labs(color="VEGFR2") +
  lightness(scale_color_brewer(palette="Greens"),scalefac(0.8)) + 
  guides(color = guide_legend(order=2)) +
  new_scale_color() +
  geom_point(data = nrp1, aes(x = "NRP1", y = koff, colour = Reference), size = 7, position = position_jitter(width=0.1), height=0) +   # Add NRP1 koff plot
  geom_point(data = nrp1, aes(x = "NRP1", y=rm_nrp1_koff$b), shape = 95, size=20, colour = "darkred") +   # Add NRP1 koff plot
  labs(color="NRP1") +
  lightness(scale_color_brewer(palette="Oranges"),scalefac(0.8)) + 
  guides(color = guide_legend(order=3)) +
  geom_errorbar(data = bar_data_koff, aes(x = Source, ymin = lb, ymax = ub), colour = bar_data_koff$color, width=0.2, linewidth=1) +
...
dev.off()
save_as_pdf(sprintf("%s/koff", results_path), width=6)
